...
class Waterfall(object):
    ...
    def update(self, *args):
        ...
        for scan_num, start_ind in enumerate(range(0, NUM_SCANS_PER_SWEEP*NFFT, NFFT)):
            ...
            fftResult = [0] * len(psd_scan)
            for i in range(len(psd_scan)):
                fftResult[i] = psd_scan[i]
                
            q2.put(fftResult)
            
            print(str(q2.qsize()) + " FFT results in the queue")

            if(q2.qsize() == 200):
                clearQueue(q2)
                print("FFT result queue cleared")
    ...
    while True:
        ...
        while True:
            if not q2.empty():
                ...
                fftResults = q2.get()
                buf = struct.pack('%sf' % len(fftResults), *fftResults)
